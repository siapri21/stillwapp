...
import { matches } from '../data/mock.ts'
    ...
    <>
      ...
      <main className="mx-auto w-full max-w-6xl px-4 pb-28 pt-2 md:px-6">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <span className="rounded-full bg-violet-600 px-4 py-2 text-sm font-semibold text-white">All Matches</span>
          <span className="rounded-full bg-white px-4 py-2 text-sm font-semibold text-[var(--sw-muted)] ring-1 ring-black/5">Design</span>
          <span className="rounded-full bg-white px-4 py-2 text-sm font-semibold text-[var(--sw-muted)] ring-1 ring-black/5">Coding</span>
        </div>
          ...
          {matches.map((match) => (
            <article key={match.id} className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-black/5">
              <div className="flex items-start gap-3">
                <AvatarImage name={match.name} size={128} rounded="2xl" className="h-16 w-16 shrink-0" />                <div className="min-w-0 flex-1">
        ...
        <Link
          to="/auth"
          className="fixed bottom-24 right-5 z-40 inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-400 text-black shadow-lg md:bottom-8"
          aria-label="Ajouter"
        >
